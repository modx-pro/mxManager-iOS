...
class Utils: NSObject {
    ...
    class func showSpinner(view: UIView, animated: Bool = true) {
        ...
        spinner.bezelView.color = UIColor.clearColor()
        if #available(iOS 9.0, *) {
            UIActivityIndicatorView.appearanceWhenContainedInInstancesOfClasses([MBProgressHUD.self]).color = UIColor.blackColor()
        }
    }
