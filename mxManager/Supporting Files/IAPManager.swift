...
extension IAPManager { // Store file managment
    ...
    func purchasedItemsFilePath() -> String {
        let documentsDirectory = NSSearchPathForDirectoriesInDomains(NSSearchPathDirectory.CachesDirectory, NSSearchPathDomainMask.UserDomainMask, true).first! as NSString
    ...
    func savePurchasedItems() {
        ...
        do {
            try data.writeToFile(purchasedItemsFilePath(), options: [.DataWritingAtomic, .DataWritingFileProtectionComplete])
        }
    ...
    func clearPurchasedItems() {
        let data = NSKeyedArchiver.archivedDataWithRootObject(NSArray())
        do {
            try data.writeToFile(purchasedItemsFilePath(), options: [.DataWritingAtomic, .DataWritingFileProtectionComplete])
        }
        catch let error as NSError {
            print("Failed to clear purchased items: \(error)")
        }
    }
}